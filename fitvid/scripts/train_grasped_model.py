...
def main(argv):
    ...
    # print the models:
    # print("FitVid Model: ", model)
    # print("Grasped Model: ", grasped_model)
    ...
    for epoch in range(resume_epoch + 1, num_epochs):
        ...
        with torch.no_grad():
            ...
            for iter_item in enumerate(tqdm(test_data_loader)):
                ...
                epoch_bce.append(metrics["ag/loss/bce"].item())
                eval_grasped_preds
                ...
                if (
                    ...
                    wandb.log(wandb_log)
                    break
            test_bce.append(np.mean(epoch_bce))
        print(f"Test BCE: {np.mean(epoch_bce)}")

        if test_bce[-1] == np.min(test_bce):
        ...
        for iter_idx in tqdm(range(FLAGS.train_epoch_max_length)):
            ...
            inputs = batch["video"], batch["actions"], batch['grasped']
